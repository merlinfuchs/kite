...
SELECT plugin_id, app_id, enabled, config, created_at, updated_at FROM plugin_instances WHERE app_id = $1 AND plugin_id = $2
`
...
DO UPDATE SET 
    ...
    config = $4, 
    updated_at = $6 
RETURNING plugin_id, app_id, enabled, config, created_at, updated_at
`
