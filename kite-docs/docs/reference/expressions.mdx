...
sidebar_position: 5
---
import ParametrizedCodeBlock from '@site/src/components/ParametrizedCodeBlock';
...
```
This will return the value of a field in the JSON response of a HTTP request block.

Fill in the values below to customize your expression.


<ParametrizedCodeBlock
  fields={[
    { name: 'block', label: 'Block ID', default: 'owlspush' },
    { name: 'field', label: 'Field Name', default: 'somefield' },
  ]}
  language="python"
  template={`{{ fromJSON(result('{{block}}').body()){{field}} }}`}
/>
