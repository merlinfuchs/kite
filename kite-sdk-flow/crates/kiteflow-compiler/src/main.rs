...
fn main() -> Result<()> {
    ...
    let wasm = generate_module(flow, optimize)?;
...
fn generate_module(flow: Vec<u8>, optimize: bool) -> Result<Vec<u8>> {
    let wasm = include_bytes!(concat!(env!("OUT_DIR"), "/engine.wasm"));

    let wasi = WasiCtxBuilder::new()
        .stdin(Box::new(ReadPipe::from(flow)))
        .inherit_stdout()
        .inherit_stderr()
        .build();
    ...
    let wasm = Wizer::new()
        ...
        .run(wasm)
        .map_err(|_| anyhow!("Flow compilation failed"))?;
