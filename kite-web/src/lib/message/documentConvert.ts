...
import type {
  ...
  MessageComponentContainerChild,
...
import type { messageSchema as restoreMessageSchema } from "./schemaRestore";
import { getUniqueId } from "@/lib/utils";
import type { DocumentData, Node, NodeId } from "./document";

/** A message as the lenient restore schema parses it, which is what the editor loads. */
export type RestoredMessage = z.infer<typeof restoreMessageSchema>;

type RestoredComponent = RestoredMessage["components"][number];
type RestoredEmbed = RestoredMessage["embeds"][number];
type RestoredSection = Extract<RestoredComponent, { type: 9 }>;
type RestoredGalleryItem = Extract<
  RestoredComponent,
  { type: 12 }
>["items"][number];
...
export function fromMessage(message: RestoredMessage): DocumentData {
  ...
  function addEmbed(embed: RestoredEmbed, parentId: NodeId): NodeId {
    const id = nodeId();
  ...
  function addOption(
    option: RestoredSelectMenuOption,
    parentId: NodeId
  ): NodeId {
    const id = nodeId();
    nodes[id] = {
  ...
  function addGalleryItem(item: RestoredGalleryItem, parentId: NodeId): NodeId {
    const id = nodeId();
    nodes[id] = {
  ...
  function addComponent(
    component:
      | RestoredComponent
      | NonNullable<RestoredSection["accessory"]>
      | Extract<RestoredComponent, { type: 1 }>["components"][number],
    parentId: NodeId
  ): NodeId {
    const id = nodeId();
    ...
    switch (component.type) {
      ...
      case 9: {
        ...
        const accessoryId = component.accessory
          ? addComponent(component.accessory, id)
          : null;
        nodes[id] = {
          type: "section",
          id,
...
const cache = new WeakMap<
  Record<NodeId, Node>,
  { rootId: NodeId; converted: ConvertedMessage }
>();

/**
 * Builds the message payload from the document, along with the mapping between
...
export function toMessage(state: DocumentData): ConvertedMessage {
  ...
  if (cached && cached.rootId === state.rootId) {
          ...
          components: componentNode.childIds.map(
            (childId, i) =>
              component(
                childId,
                `${path}.components.${i}`
              ) as MessageComponentContainerChild
          ),
  ...
  const message: Message = {
    ...
    components: root.componentIds.map(
      (id, i) => component(id, `components.${i}`) as MessageComponent
    ),
  };

  const converted = { message, pathToId, idToPath };
  cache.set(state.nodes, { rootId: state.rootId, converted });
  return converted;
}

type RestoredActionRow = Extract<RestoredComponent, { type: 1 }>;
type RestoredButton = Extract<
  RestoredActionRow["components"][number],
  { type: 2 }
>;
 ...
 */
export function interactiveRows(message: RestoredMessage): RestoredActionRow[] {
  const rows: RestoredActionRow[] = [];
  let loose: RestoredButton[] = [];
  ...
  function walk(component: RestoredComponent) {
    switch (component.type) {
      case 1:
        flushLoose();
        rows.push(component);
